...
const defaultLibraryRoot = fileURLToPath(new URL('./runtime/library/', import.meta.url));
const libraryRoot = options.get('library-root') || process.env.DOWNPIC_LIBRARY_ROOT || defaultLibraryRoot;
const environment = await loadDeepSeekEnvironment(defaultRuntimeRoot());

if (!environment.DEEPSEEK_API_KEY) {
  console.warn('未配置 DeepSeek 凭据，反推提示词不可用。请配置后通过 plugin-prototype/scripts/start-prototype.ps1 启动，或设置 DEEPSEEK_API_KEY。');
}

createPrototypeServer({ libraryRoot, environment })
  .listen(port, '127.0.0.1', () => {
    console.log(`Prototype ready: http://127.0.0.1:${port}/plugin-prototype/`);
    console.log(`素材根目录：${libraryRoot}`);
  });
